...
namespace DialogueCreator3
{
    ...
    {
        private node N;
        public List<node> InactiveNodes;                //BIGGEST ISSUE RIGHT NOW   Opt1: Use a greater class to get the inactive nodes properly
        public KeyValuePair<int, node> SelectedNodeAnswer;
        public KeyValuePair<int, node> SelectedNodeQuestion;
        ...
        public void SelectedAnswer(object sender, EventArgs e)
        {
            ...
            if(temp == null)
            {
                foreach(node n in InactiveNodes)
                {
                    if(n.ID == tmp.ID)
                    {
                        temp = n;
            ...
            if(tmp.SelectedNode == 0) // Input (Question)
            {
                SelectedNodeQuestion = new KeyValuePair<int, node>(tmp.selected, temp);
                if (SelectedNodeAnswer.Value != null)
                {
                    node a = SelectedNodeAnswer.Value;
                    a.Link(SelectedNodeAnswer.Key, temp);
                    Reset();
                }
            }
            else // Outputs (Answers)
            {
                SelectedNodeAnswer = new KeyValuePair<int, node>(tmp.SelectedNode, temp);
                if (SelectedNodeQuestion.Value != null)
                {
                    temp.Link(SelectedNodeQuestion.Key, temp);
                    Reset();
                }
            }
        }

        public void SelectedQuestion(object sender, EventArgs e) // Change structure (to make sure we still have SelectedNodeQuestion & Answer but not 2 events)
        {
            Block tmp = (Block)sender;
            node temp = (N.Find(tmp.ID));
            if(temp == null)
        ...
        public void Delink(object sender, EventArgs e)
        {
            ...
            {
                if(tmp.NextNodes.Count() >= b.selected+1)
                {
                    if (tmp.NextNodes[b.selected + 1] != null)
                    {
                        tmp.NextNodes.RemoveAt(b.selected + 1); // Check back the link() later (possible issue with Add() when you choose not to link the first output in order)
            ...
            else
            {
                ...
                for(int i=0;i<tmp.Parent.NextNodes.Count();i++)
                {
                    if(tmp.Parent.NextNodes[i] == tmp) { tmp.Parent.NextNodes.RemoveAt(i);tmp.Parent.Answers.RemoveAt(i); break; }
                }
            }
        }
        ...
        private void Form1_MouseClick(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right)
            {
                node n = new node(0);
                InactiveNodes.Add(n);
                InactiveNodes.Last().Form = this;
                Point p = ArrangePlacing(new Point(e.X-50, e.Y-50));
                Block tmp = InactiveNodes.Last().SetBlock(p, new Size(new Point(100, 100)),n.ID);
                tmp.ID = n.ID;
                tmp.SelectedAnswer += new EventHandler(SelectedAnswer);
        ...
        private void OpenFile(string s)
        {
            ...
            foreach(KeyValuePair<KeyValuePair<int,int>,int> a in NewLinks)
            {
                ...
                {
                    if(a.Key.Key == InactiveNodes[i].ID)
                    {
                        for(int j=0;j<InactiveNodes.Count();j++)
                        {
                            if(a.Value == InactiveNodes[j].ID)
                            {
                                InactiveNodes[i].InlineLink(a.Key.Value+1, InactiveNodes[j]);
                                SelectedNodeAnswer = new KeyValuePair<int,node>(a.Key.Value+1,InactiveNodes[i]);
                                SelectedNodeQuestion = new KeyValuePair<int,node>(-1,InactiveNodes[j]);
                                Reset();
                                break;
                            }
                        }
        ...
        private void deleteToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (Id > 0)
            {
                ...
                for (int i = 0; i < InactiveNodes.Count(); i++)
                {
                    if (InactiveNodes[i].ID == Id) { InactiveNodes.RemoveAt(i); break; }
                }
            }
    ...
    public class node
    {
        ...
        public void SetNode(int index, string Answer, int NextId)
        {
            NextNodes[index] = new node(0);
            NextNodes[index].Form = Form;
        }
        ...
        public void Link(int Answer, node Second) // Make sure that when the link is over, if(nbAnswers<5) nbAnswers++; and all that goes with it
        {
            ...
            if (NextNodes.Count() <= Answer)
            {
                NextNodes.Add(Second);
        ...
        public string Run()
        {
            ...
            if (Answers.Count() == 0) { tmp = tmp.Remove(tmp.Length - 9); }
                ...
                {
                    tmp += "(AnswerText=\"";
                    tmp += Answers[i] + "\",NextQuestionId=" + NextNodes[i].ID.ToString() + "),";
                    //tmp += NextNodes[i].Run();
                    //tmp += ",";
                }
        ...
        public void Delete()
        {
            ...
            {
                //Should add them back to the InactiveNodes
